...
默认拉起项目时，使用的是 `.env` 里配置的在线 API 模型。只要执行 `docker compose up --build`，就可以完成初始化、打开 Demo 页面并跑通常规启动检查，不需要先准备 Ollama。
...
2. 按你的模型接入方式修改 `.env`。
   ...
   对于默认启动路径，这里配置的是在线 API 模型即可。直接执行 `docker compose up --build` 就能把项目拉起并完成初始化、页面联调和常规 Demo 测试。仓库里保留的 Ollama 地址只服务于可选的本地模型路径。
...
4. 如果要连同本地 Ollama 一起启动：
   ...
   只有在你明确要测试可选的本地模型路径时，才需要启用这个 profile。它不是默认初始化流程的一部分。
